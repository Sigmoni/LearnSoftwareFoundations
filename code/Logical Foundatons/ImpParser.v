...
Definition parserNumber (xs : list token)
...
with parseSumExp (steps : nat)
  ...
  | S steps' =>
    ...
    ' (es, rest') <- 
        many (fun xs =>
                TRY ' (e, rest') <- 
                    firstExpect "+" (parseProductExp steps') xs ;;
                    SomeE ( (true, e), rest') 
                OR  ' (e, rest') <- 
                    firstExpect "-" (parseProductExp steps') xs ;;
                    SomeE ( (false, e), rest'))
...
Fixpoint parseSimpleCommand (steps : nat) 
  ...
  | S steps' =>
    ...
    TRY ' (e, rest) <- 
        ...
        ' (c, rest') <- 
            firstExpect "then"
                        (parseSequencedCommmand steps') rest ;;
        ' (c', rest'') <-
            firstExpect "else" 
                        (parseSequencedCommmand steps') rest' ;;
        ' (tt, rest''') <- 
  ...
  with parseSequencedCommmand (steps : nat) 
                              (xs : list token) :=
    match steps with
    | 0 => NoneE "Too many recursive calls"
    | S steps' => 
...
Definition parse (str : string) : optionE com := 
  ...
  match parseSequencedCommmand bignumber tokens with 
  | SomeE (c, []) => SomeE c 
